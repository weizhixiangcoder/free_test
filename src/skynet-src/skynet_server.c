...
struct skynet_context {			//skynet中的环境
	void * instance;
...
static struct skynet_node G_NODE;		//本文件   几个主要的线程的私有数据	各个线程pthread_setspecific的值不一样
...
skynet_context_new(const char * name, const char *param) {
	struct skynet_module * mod = skynet_module_query(name);		//加载名为name的模块
...
skynet_globalexit(void) {
	pthread_key_delete(G_NODE.handle_key);		//释放私有数据
}
